...
function checkoutOrder_Mobile() {
    ...
    //setTimeout({
    //    checkPromotionMoney(totalOrder);
    //}, 100)
...
function checkPromotionMoney(totalOrder) {
    let request = { TotalMoney: totalOrder };
    $.ajax({
        url: "http://api.anhat.vn/api/Order/CheckPromotionMoney",
        method: "POST",
        data: JSON.stringify(request),
        dataType: 'json',
        contentType: "application/json",
        success: function (result, status, jqXHR) {
            $(".promotion__total__shoping__checkout").html(`Tiền được tích lũy: <span>${formatter.format(result.Money)}</span>`);
        },
